...
use Autopoets\NewsBundle\Entity\News;
use Autopoets\NewsBundle\Form\NewsType;
...
class NewsController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        foreach($entities as $key => $entity) {
            if(file_exists('images/img' . $entity->getId() . '.jpg')) {
                $entities[$key]->image = 'images/img' . $entity->getId() . '.jpg';
            } else {
    ...
    public function createAction(Request $request)
    {
       // var_dump('test');die;
         ...
         $news = new News();
        $form = $this->createCreateForm($news);
    ...
    private function createCreateForm(News $entity)
    {
        ...
        $form = $this->createFormBuilder()
            ->setAction($this->generateUrl('news_create'))
            ->setMethod('POST')
            ->add('title', 'text')
